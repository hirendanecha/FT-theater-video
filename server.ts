...
export function app(): express.Express {
  ...
  const distFolder = join(process.cwd(), 'tube-dist/ft-theater-video/browser');
  ...
  const template = fs
    .readFileSync(
      path.join(join(process.cwd(), 'tube-dist/ft-theater-video/browser'), 'index.html')
    )
    .toString();
    ...
    res.render(
      ...
      async (err, html) => {
        ...
        var seo: any = {
          ...
          url: 'https://video.fuck.theater' + params,
          keywords: 'FuckTheaterVideo',
        };
        if (
          ...
          const talent = {
            name: post?.title || post?.albumname || 'fuck.theater Post',
            description: pdhtml?.textContent || 'Post content',
            image: post?.thumbfilename || post?.metaimage || post?.imageUrl || 'https://video.fuck.theater/assets/img/cover.png',
          };
          seo.title = talent.name;
          seo.description = strip_html_tags(talent.description);
          seo.image = talent.image;
